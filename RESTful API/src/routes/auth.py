...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)) -> UserResponse:
    """
    Register a new user.
...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db))-> TokenModel:
    """
...
async def confirmed_email(token: str, db: Session = Depends(get_db)) -> dict:
    ...
    :param db: The database session.
    :type db: Session
    :return: A message indicating whether the email is confirmed.
    :rtype: dict
    """
...
async def refresh_token(credentials: HTTPAuthorizationCredentials = Security(security), db: Session = Depends(get_db)) -> TokenModel:
    """
